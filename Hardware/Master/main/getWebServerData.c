  ...
  while (1) {
    if (xSemaphoreTake(sim800_uart_mutex, portMAX_DELAY) == pdTRUE) {
      ...
      snprintf(command, sizeof(command),
               "AT+HTTPPARA=\"URL\",\"http://%s:5000/api/"
      ...
      if (json_start) {
        ...
        ESP_LOGI("CONFIG",
                 "Parsed Config: TEMP [%d-%d], HUM [%d-%d], ALERTS: %s, GPS "
                 "REF: [%.6f, %.6f]",
                 new_config.min_temp, new_config.max_temp,
                 new_config.min_humidity, new_config.max_humidity,
                 new_config.is_alerts_on ? "ON" : "OFF", new_config.latitude,
      ...
      vTaskDelay(pdMS_TO_TICKS(30000)); // 30 min
