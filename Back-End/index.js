...
console.log(address);
    if (!location) {
      return res
        .status(404)
        .json({ error: "No suitable location found in data" });
    }
...
app.get("/api/public_alert-config", async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      isEcoModeON:settings.is_eco_mode_on,
...
app.get('/api/user', async (req, res) => {
  ...
  try {
    ...
    res.json(user); // send full user object (except password)
  } catch (err) {
...
app.put("/api/user", async (req, res) => {
  ...
  try {
    const updateData = { full_name, email, phone_number };

    // Only hash and update password if a new one is provided
    if (password && password.trim() !== "") {
      const hashedPassword = await bcrypt.hash(password, 10); // same as registration
...
app.get("/api/admin/specific-user", async (req, res) => {
  try {
    const { email, phone_number } = req.query;

    if (!email && !phone_number) {
      return res.status(400).json({ error: "Please provide email or phone_number" });
    }
    ...
    // Find one user matching either email or phone number
    const user = await User.findOne(filter).select("-password"); // exclude password
...
// Get all users sorted by registration date (recent first)
app.get("/api/admin/users-by-date", async (req, res) => {
  try {
    const users = await User.find()
      .select("-password")            // exclude passwords
      .sort({ created_at: -1 });      // -1 = descending (recent first)
...
//// GET all beehives with user info
app.get("/api/admin/all-beehives", async (req, res) => {
  try {
    const beehives = await Beehive.find()
      .populate("user_id", "full_name email phone_number") // include selected user fields
      .sort({ created_at: -1 }); // optional: recent first
