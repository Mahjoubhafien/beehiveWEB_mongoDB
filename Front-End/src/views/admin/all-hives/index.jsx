...
// Calculate total pages
  const totalPages = Math.ceil(listOfHives.length / itemsPerPage);
  
  // Get current hives to display
  const indexOfLastHive = currentPage * itemsPerPage;
  const indexOfFirstHive = indexOfLastHive - itemsPerPage;
  const currentHives = listOfHives.slice(indexOfFirstHive, indexOfLastHive);
  
  // Change page
  const paginate = (pageNumber) => setCurrentPage(pageNumber);
  
  // Reset to first page when listOfHives changes
  useEffect(() => {
    setCurrentPage(1);
  }, [listOfHives]);
    ...
    <div>
      ...
      <AnimatePresence>
        {isSlideclicked && (
          ...
          >
            <AddHiveArea AddHive={AddHiveHandler} />
          </motion.div>
        )}
      </AnimatePresence>
