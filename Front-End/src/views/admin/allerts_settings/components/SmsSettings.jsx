...
import eco from "assets/img/avatars/environment.png"

function SmsAlertsTable(props) {
  const [smsEnabled, setSmsEnabled] = useState();
  const [isEcoModeON, setIsEcoModeON] = useState(false);
...
useEffect(() => {
  const fetchSettings = async () => {
    try {
      ...
      setIsEcoModeON(data.isEcoModeON);
...
/>
    <label className="relative inline-flex cursor-pointer items-center">
      <input
        ...
        className="peer sr-only"
        checked={isEcoModeON}
        onChange={() => setIsEcoModeON(!isEcoModeON)}
      />
      <div className="peer h-6 w-11 rounded-full bg-gray-200 
        ...
        dark:border-gray-600 dark:bg-gray-700 dark:peer-focus:ring-blue-800"></div>
    </label>
  </div>
</div>
